...
namespace AoC.Util
{
        ...
        where TNode : AStarNode
    {
        ...
        public Base.Vec2 Start { get; private set; }
        public Base.Vec2 End { get; private set; }

        public AStar(int maxX, int maxY)
        {
            Initialized = false;
            Start = new Base.Vec2(-1, -1);
            End = new Base.Vec2(-1, -1);
            MaxX = maxX;
            MaxY = maxY;
        ...
        public void Process(CanUseNode canUseNode = null, IsEnd isEnd = null, AdjustLength adjustLength = null, GetPriority getPriority = null)
        {
            ...
            {
                Base.Vec2 curPos = priorityQueue.Dequeue();
                ...
                foreach (Base.Vec2 neighborOffset in NeighborOffsets)
                {
                    Base.Vec2 nextPos = curPos + neighborOffset;
        ...
        {
            char[,] path = Input;
            Base.Vec2 curPos = End;
            while (curPos != null)
            {
                TNode curNode = Nodes[curPos.X, curPos.Y];
                Base.Vec2 prevPos = curNode.Prev;
                ...
                Base.Vec2 direction = curPos - prevPos;
